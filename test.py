...
def get_transforms(args):
    ...
    #         # mt.AsDiscreteD(keys=['seg'], to_onehot=True, n_classes=4),
    #     ]
    # )
...
def main(args):
    ...
    images = sorted(glob(pjoin(data_folder, 'train_multi', 'images', '*.npz')))
    segs = sorted(glob(pjoin(data_folder, 'train_multi', 'masks', '*.npz')))
    ...
    # with torchprof.Profile(model, use_cuda=True) as prof_layer: 
    ...
    s2 = time.time()
    print("================= One img process: %f ms" % ((s2-s1)*1000))
    
    # print(prof_layer.display(show_events=True))
    ...
    # sum_flops = flopth(model, in_size=[3, 480, 640])
    # print(sum_flops)
    # print("+++++++++++++++++++++++++++++")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default="./cvt_model/new_model", metavar='N', help='Directory to save checkpoints')
